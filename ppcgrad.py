...
class PPCGrad:
    ...
    def project_conflicting_gradient(self, i, g_i, index, pc_grad, grads):
        random.shuffle(index)
        for i_ in index:
            g_i_g_j: torch.Tensor = torch.dot(g_i, grads[i_])
            if g_i_g_j < 0:
                pc_grad[i] -= g_i_g_j * grads[i_] / (grads[i_].norm() ** 2)
    ...
    def project_conflicting(self, grads: List[torch.Tensor], has_grads: List[torch.Tensor]) -> torch.Tensor:
        ...
        pc_grad = manager.list(pc_grad)
        grads = manager.list(grads)
        index = list(range(len(grads)))
        process_list=[]
        for i, g_i in enumerate(pc_grad):
            process=multiprocessing.Process(target=self.project_conflicting_gradient, args=(i, g_i, index, pc_grad, grads))
            process.start()
